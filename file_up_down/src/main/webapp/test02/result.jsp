...
<title>이미지 파일 출력 : result.jsp => FileDownload (downlaod.do)</title>
	<pre>
		1. 이미지 파일 표시창에서 <img> 태그의 scr 속성에 
			다운로드할 요청 url pattern명(download.do)을 이용하여 요청
			파일이름을 get방식으로 전송
		2. 다운로드한 이미지 파일을 <img> 태그에 출력
		3. <a href=""></a>를 이용하려 사용자가 이미지를 클릭 했을 때 다운로드 할 수 있도록...
		4. 파일 전체를 로컬PC의 다운로드 폴더에 저장
		
		1. 다운로드할 파일 이름을 추출
		2. 파일 이름으로 서블릿에서 이미지를 다운로드하여 출력
		3. 파일 이름으로 서블릿에서 이미지를 다운로드하여 출력
		4. 이미지 파일 다운로드...
	</pre>
	<!-- 1. -->
	...
	<!-- 3. -->
	<c:if test="${not empty file2}"> <!-- get방식 -->
